...
박스 모델은 총 네 부분(영역)으로 이루집니다. 패딩, border, margin, content입니다. content는 텍스트나 이미지가 들어가있는 박스의 실질적인 부분으로, 색상이나 너비, 높이 등을 지정할 수 있습니다. 

패딩은 content와 border 사이의 간격입니다. 패딩은 눈에 보이지 않습니다.

border는 content와 패딩 주변을 감싸는 테두리입니다. margin은 테두리와 이웃하는 요소 사이의 간격이고, margin 역시 패딩처럼 눈에 보이지 않습니다.
